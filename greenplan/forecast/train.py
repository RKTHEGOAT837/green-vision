...
def _champion_line(cfg: Any) -> str:
    """The deployed forecaster's own held-out skill, read from the artefact
    that recorded it rather than typed into a format string.

    `training/backtest.py` scores the statistical forecaster with the SAME
    skill definition used here (baseline_norm_error - norm_error), so the two
    numbers are directly comparable — which is the only reason printing them
    side by side is honest. If the store is missing or predates the field, say
    so instead of substituting a number.
    """
    meta = Path(cfg.training.memory_path).with_name("memory.meta.json")
    try:
        rep = json.loads(meta.read_text(encoding="utf-8")).get("last_report", {})
        first = rep["skill_vs_baseline_first_half"]
        second = rep["skill_vs_baseline_second_half"]
        mean = (first + second) / 2
        return (
            f"(the DEPLOYED statistical forecaster scores {mean:+.3f} by this same "
            f"definition, over {rep.get('iterations', '?')} held-out tasks — {meta})"
        )
    except Exception:
        return f"(no comparable champion score on disk: {meta} missing or incomplete)"
...
def main() -> int:
    ...
    print(
        ...
        # Was a hardcoded "-0.333" for the LLM, which no artefact in this repo
        # backs. The statistical champion's figure IS backed — it is read from
        # the memory store this same skill definition wrote — so print that.
        f"{_champion_line(cfg)}\n"
        f"verdict: {verdict}\n"
